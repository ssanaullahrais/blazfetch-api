...
- **SSRF protection:** links are resolved and refused when they point at loopback, private, link-local or reserved
  addresses (IPv4 and IPv6, including IPv4-mapped forms). Redirects are followed by the server itself and every hop is
  checked again, so an allowed host cannot bounce a request to an internal address.
...
- **Limits:** rate limits per visitor, concurrency limits, a maximum download size, timeouts on every external process,
