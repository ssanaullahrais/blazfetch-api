...
| A link fails with "The extractor failed" on the server, but works from your own computer | The site blocks the server's IP or yt-dlp is outdated (Reddit does both). Run `sudo yt-dlp -U`, then test with `yt-dlp -J "<link>"` on the server. |
| The site stops answering (Nginx 502) while several videos are prepared | The server ran out of memory or CPU. Conversions are limited to half the CPU cores at once by default (`MAX_CONCURRENT_CONVERSIONS`); on a 1-2 GB server set `MAX_CONCURRENT_CONVERSIONS=1` and `MAX_CONCURRENT_DOWNLOADS_GLOBAL=3`, and check `pm2 logs` and `dmesg -T \| grep -i killed` for the cause. |
| Message "The server is busy or you already have a download running" | Each visitor may run 1 video download and 1 audio download at a time, counted separately (`MAX_CONCURRENT_DOWNLOADS_PER_GUEST`, applied per kind) and the server 10 (`MAX_CONCURRENT_DOWNLOADS_GLOBAL`). It clears when the running download ends. Raise the limits only if the server has CPU to spare. |
