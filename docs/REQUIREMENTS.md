...
- Debian 12
- Any other reasonably current Linux distribution with Node.js 20+, PostgreSQL 14+, and Python 3
...
Set `DATABASE_URL=postgres://blazfetch:change-me@localhost:5432/blazfetch` in `.env`, then run:
...
deployments should set: `APP_ENV=production`, `DATABASE_URL`, `APP_URL`, `CORS_ALLOWED_ORIGINS`
...
- `443` / `80` — Nginx (public)
- `5432` — PostgreSQL (bind to localhost only unless using a managed/remote DB)
...
Do not expose port 4000 (the app) or 5432 (Postgres) publicly — both should only be reachable via
localhost/Nginx.
...
- Back up PostgreSQL only — it holds metadata cache, stats, and job records, never media files.
  `pg_dump` on a schedule (e.g. daily) is sufficient given the low write volume relative to a
...
```bash
npm run diagnostics          # Node/Postgres/yt-dlp/ffmpeg/ffprobe status in one command
curl localhost:4000/health/ready
yt-dlp --version
ffmpeg -version
sudo -u postgres psql -c "\l"           # confirm the blazfetch database exists
