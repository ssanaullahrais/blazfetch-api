...
const envSchema = z.object({
  ...
  // which is otherwise unavailable anonymously. Used when INSTAGRAM_INSTALOADER_SESSION_PATH is
  // not set (Instaloader, below, is preferred when both are configured).
  INSTAGRAM_COOKIES_PATH: z.string().optional().default(''),

  // Optional, preferred profile-listing engine: Instaloader (https://instaloader.github.io/),
  // via scripts/python/instaloader_bridge.py. Requires `pip install instaloader` and a session
  // file the operator creates themselves with `instaloader --login <username>` (never generated
  // or obtained by this backend). INSTAGRAM_INSTALOADER_SESSION_USERNAME must match the account
  // that session file belongs to (Instaloader validates the pairing).
  PYTHON_PATH: z.string().default('python'),
  INSTAGRAM_INSTALOADER_SESSION_PATH: z.string().optional().default(''),
  INSTAGRAM_INSTALOADER_SESSION_USERNAME: z.string().optional().default(''),
  INSTALOADER_TIMEOUT_MS: z.coerce.number().default(30000),
